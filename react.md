...
### 정리
라이프사이클 메서든ㄴ 컴포넌트 상태에 변화가 있을 때마다 실해하는 메서드이고 이 메서드들은 서브파티 라이브러리를 사용하거나 DOM을 직접 건드려야 하는 상황에서 유용합니다. 추가로 컴포넌트 업데이트의 성능을 개선할 때는 shouldComponentupdate가 중요하게 사용된다. 
